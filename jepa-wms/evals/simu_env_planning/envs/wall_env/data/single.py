...
class DotDataset:
    ...
    def render_location(self, locations: torch.Tensor):
        ...
        locations = locations.unsqueeze(-2).unsqueeze(-2)  # add dims for compatibility with c
        img = torch.exp(-(c - locations).norm(dim=-1).pow(2) / (2 * self.config.dot_std * self.config.dot_std)) / (
