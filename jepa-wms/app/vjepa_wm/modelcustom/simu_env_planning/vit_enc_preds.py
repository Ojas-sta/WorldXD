...
class EncPredWM(nn.Module):
    ...
    def unroll(self, z_ctxt, act_suffix=None, debug=False):
        ...
        T_v = vid_feats_prefix.shape[1]
        
        # Pad prop_feats to match T_v
        if prop_feats_prefix is not None and prop_feats_prefix.shape[1] < T_v:
            pad_len = T_v - prop_feats_prefix.shape[1]
            last_frame = prop_feats_prefix[:, -1:]
            prop_feats_prefix = torch.cat([prop_feats_prefix, last_frame.expand(-1, pad_len, *last_frame.shape[2:])], dim=1)
        ...
        if T_v > 1:
            act_feats_prefix = torch.zeros(
                B, T_v - 1, *act_feats_suffix.shape[2:], 
                device=act_feats_suffix.device, dtype=act_feats_suffix.dtype
            )
        else:
            act_feats_prefix = None

        for h in range(T):
            new_act_feats = act_feats_suffix[:, h : h + 1]
            if h == 0:
                if act_feats_prefix is not None:
                    act_feats = torch.cat([act_feats_prefix, new_act_feats], dim=1)
                else:
        ...
        elif isinstance(z_ctxt, dict):
            # Plain dict input — return plain dict same as TensorDict
            vid_feats = rearrange(vid_feats, "b t ... -> t b ...")
            if prop_feats is not None:
                prop_feats = rearrange(prop_feats, "b t ... -> t b ...")
            return {"visual": vid_feats, "proprio": prop_feats}
        elif isinstance(z_ctxt, torch.Tensor):
            vid_feats = rearrange(vid_feats, "b t ... -> t b ...")
            return vid_feats
    ...
    def encode(self, obs, act=True):
        ...
        if isinstance(obs, TensorDict) or isinstance(obs, dict):
            ...
            prop_param = next(self.model.proprio_encoder.parameters())
            trans_proprio = trans_proprio.to(device=prop_param.device, dtype=prop_param.dtype)
        ...
        # Cast to encoder's weight dtype to prevent Half/Float mixed errors
        enc_param = next(self.model.encoder.parameters())
        trans_visual = trans_visual.to(device=enc_param.device, dtype=enc_param.dtype)
